...
public class OfferServiceImpl implements OfferService {
	...
	public Offer engageOffer(String token, int offerId, int empId) {
		LOGGER.info("Inside engage offer service");
		AuthResponse authResponse = authClient.verifyToken(token).getBody();
		if (authResponse.isValid()) {
			Offer offer = offerRepository.findById(offerId).orElse(null);
			if (offer != null) {
				offer.setEngagedDate(LocalDate.now());
				offer.setEngagedEmpId(empId);
				offerRepository.save(offer);
	...
	public Offer editOffer(String token, Offer offerNew, int offerId ) {
		LOGGER.info("Inside edit offer service");
		AuthResponse authResponse = authClient.verifyToken(token).getBody();
		if (authResponse.isValid()) {
			OfferCategory category = offerCategoryRepository.findById(offerNew.getOfferCategory().getOffCategoryId())
					.get();
			if (category != null) {
				Offer offer = offerRepository.findById(offerId).get();
				if(offer.getOfferCategory().getOffCategoryName()!=category.getOffCategoryName())
				{
					offer.setLikes(0);
					offer.setLikedByEmployee(new HashSet<Integer>());
				}
				offer.setOfferCategory(category);
				offer.setClosedDate(offerNew.getClosedDate());
				offer.setOpenedDate(offerNew.getOpenedDate());
				offer.setOfferName(offerNew.getOfferName());
				offerRepository.save(offer);
				return offer;
			} else {
				LOGGER.error("No category");
	...
	public List<Offer> getOfferByPostedDate(String token, String date) throws ParseException {
		...
		AuthResponse authResponse = authClient.verifyToken(token).getBody();
		if (authResponse.isValid()) {
			List<Offer> offerList = offerRepository.findByPostedDate(LocalDate.parse(date));
			return offerList;
		} else {
			LOGGER.error("Invalid token");
			throw new InvalidUserException("Invalid User");
	...
	public Offer updateLikes(String token, int offerid, int empid) {
		...
		if (authResponse.isValid()) {
			Offer offer = offerRepository.findById(offerid).orElse(null);
			if (offer != null) {
				offer.getLikedByEmployee().add(empid);
				offer.setLikes(offer.getLikedByEmployee().size());
				offerRepository.save(offer);
				pointClient.refreshPoints(token, empid);
			}
			return offer;
